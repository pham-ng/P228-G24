...
const metrics = JSON.parse(readFileSync("bench/baselines/kiosk-validation/FINAL-LOCAL-EVAL-METRICS.json", "utf8"));
const cases = JSON.parse(readFileSync("bench/baselines/kiosk-validation/FINAL-LOCAL-EVAL-CASES.json", "utf8"));

const overall = metrics.overall;
const bySplit = metrics.bySplit;
const byCategory = metrics.byCategory;
const byLanguage = metrics.byLanguage;
const byAnswerability = metrics.byAnswerability;
const usefulness = metrics.usefulness;
const taxonomy = metrics.failure_taxonomy;
const criticals = metrics.critical_failures;
const multiTurn = metrics.multi_turn;

const reportMd = `# Aurea — RELEASE-GRADE LOCAL MODEL EVALUATION REPORT
...
writeFileSync("bench/baselines/kiosk-validation/FINAL-LOCAL-PRODUCT-EVALUATION.md", reportMd);
console.log("Successfully updated FINAL-LOCAL-PRODUCT-EVALUATION.md with complete 23-point specification!");
