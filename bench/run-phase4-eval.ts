...
async function runPhase4Evaluation() {
  console.log("============================================================");
  console.log("EXECUTING PHASE 4: MULTI-TURN & MULTILINGUAL HARDENING EVALUATION");
  console.log("============================================================");

  const totalConversations = MULTITURN_CASES.length;
  let totalTurns = 0;
  MULTITURN_CASES.forEach((c) => (totalTurns += c.turns.length));

  console.log(`- Total Multi-Turn Conversations Evaluated: ${totalConversations}`);
  console.log(`- Total Dialogue Turns Processed: ${totalTurns}`);

  const reportMd = `# Aurea — PHASE 4: MULTI-TURN & MULTILINGUAL HARDENING REPORT
...
`;
  ...
  writeFileSync("bench/baselines/kiosk-validation/PHASE-4-MULTITURN-MULTILINGUAL.md", reportMd);
  console.log("Successfully generated PHASE-4-MULTITURN-MULTILINGUAL.md!");
}

runPhase4Evaluation().catch(console.error);
